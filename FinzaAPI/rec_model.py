...
- Trains once at import time on synthetic data based on a sample JSON.
- Exposes `recommend_for_payload(payload: dict, top_k: int = 3)` which:
  * uses the same style of JSON that your Monte Carlo simulator receives
  * returns top-k recommendations from the RECOMMENDATIONS list
"""
...
'''
_TRAIN_DATA = json.loads(_sample_json_text)
...
def extract_features_for_individual(ind, global_data):
    ...
    if planning_horizon is not None:
        planning_horizon = planning_horizon - current_age
    if planning_horizon is None or planning_horizon < 0:
        planning_horizon = 30

    total_portfolio = sum([p.get(k, 0.0) for k in ["cash", "taxable", "tax_deferred", "tax_free"]])
    denom = total_portfolio + 1e-9

    cash_ratio = p.get("cash", 0.0) / denom
    taxable_ratio = p.get("taxable", 0.0) / denom
    tax_deferred_ratio = p.get("tax_deferred", 0.0) / denom
    tax_free_ratio = p.get("tax_free", 0.0) / denom

    annual_savings_total = sum([a.get(k, 0.0) for k in ["cash", "taxable", "tax_deferred", "tax_free"]])
    pre_inc = ind.get("pre_retirement_income", global_data.get("pre_retirement_expenses", 0.0))
    savings_rate_est = annual_savings_total / (pre_inc + 1e-9)

    near_retirement = 1.0 if years_to_retire <= 7 else 0.0
    low_cash_buffer = 1.0 if p.get("cash", 0.0) < 0.5 * (pre_inc / 12.0 + 1e-9) else 0.0
    low_emergency = 1.0 if p.get("cash", 0.0) < pre_inc * 0.25 else 0.0
...
# --------------------------
# rule-based labeler -> synthetic training labels
# --------------------------
def heuristic_top3_recs(ind, global_data):
    ...
    retirement_age = ind.get("retirement_age", current_age)
    years_to_retire = max(0, retirement_age - current_age)
    total_port = sum([p.get(k, 0.0) for k in ["cash", "taxable", "tax_deferred", "tax_free"]])
    pre_inc = ind.get("pre_retirement_income", global_data.get("pre_retirement_expenses", 0.0))

    months_cash = p.get("cash", 0.0) / (pre_inc / 12.0 + 1e-9)
    ...
    if years_to_retire <= 7:
        scores[47] += 2.5  # contribute to 401k
        scores[64] += 2.0  # increase contributions
        scores[66] += 1.5  # diversify tax types
        scores[68] += 1.0  # consider delaying SS
        scores[60] += 1.0  # estimate expenses

    # heavy tax-deferred
    td = p.get("tax_deferred", 0.0)
    ...
    # small portfolio vs income
    if total_port < pre_inc * 0.5:
        scores[0] += 1.0
        scores[1] += 1.0
        scores[24] += 1.0

    # encourage more equities if very low taxable
    if p.get("taxable", 0.0) < 0.1 * (total_port + 1e-9):
        scores[48] += 1.2
        scores[59] += 0.8

    # always sprinkle in education / consistency
    scores[99] += 0.5
    scores[90] += 0.5
...
# --------------------------
# synthetic dataset + model training (once at import)
# --------------------------
...
def _make_training_data():
    base_templates = _TRAIN_DATA["individuals"]
    X_list = []
    Y_list = []
    num_samples = 3000

    def make_random_individual(base_ind, global_data):
        ind = json.loads(json.dumps(base_ind))  # deep copy
    ...
    for _ in range(num_samples):
        ...
        ind = make_random_individual(base, _TRAIN_DATA)
        ind["pre_retirement_income"] = ind.get(
            "pre_retirement_income", _TRAIN_DATA.get("pre_retirement_expenses")
        )
        feat = extract_features_for_individual(ind, _TRAIN_DATA)
        labels = heuristic_top3_recs(ind, _TRAIN_DATA)
...
def _train_recommender():
    ...
    for epoch in range(8):
        ...
        epoch_loss /= len(ds)
        print(f"[rec_model] Epoch {epoch+1}/8 loss: {epoch_loss:.4f}")
...
_MODEL, _DEVICE = _train_recommender()


# --------------------------
# Public API: recommend_for_payload
# --------------------------
def recommend_for_payload(payload: dict, top_k: int = 3):
    """
    Given the SAME payload you send to /simulate (RetirementInput),
    return top-k recommendations as a list of dicts:

    [
      { "id": int, "title": str, "description": str, "impact_summary": str },
      ...
    ]
    """
    individuals = [ind for ind in payload.get("individuals", []) if ind.get("name") != "Joint"]
    if not individuals:
        return []

    ind = dict(individuals[0])
    ind["pre_retirement_income"] = ind.get(
        "pre_retirement_income",
        payload.get("pre_retirement_expenses", 0.0),
    )

    feat = extract_features_for_individual(ind, payload)
    x = torch.from_numpy(feat.astype(np.float32)).unsqueeze(0).to(_DEVICE)

    with torch.no_grad():
        logits = _MODEL(x).cpu().numpy().flatten()
    probs = 1.0 / (1.0 + np.exp(-logits))

    top_idx = list(np.argsort(-probs)[:top_k])
        ...
        recs.append(
            {
                ...
                "description": RECOMMENDATIONS[idx],
                "impact_summary": "This action is prioritized based on your age, savings, and portfolio mix.",
            }
        )
    return recs